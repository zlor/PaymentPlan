...
class ChangeFileColumns extends Migration
{
    ...
    {
        Schema::table('files', function (Blueprint $table) {
            $table->text('import_msg')->nullable()->comment('载入后的信息')->change();
        });

        // 付款计划 < 多对多关联 > 文件, 文件导入生成多个付款计划； 一个付款计划由多个文件导入覆盖；
        Schema::table('payment_schedule_files', function(Blueprint $table){
            $table->text('import_msg')->nullable()->comment('载入时反馈的信息')->change();
            $table->text('import_source')->nullable()->comment('记录载入时读取到的原始信息')->change();
        });
    }
}
